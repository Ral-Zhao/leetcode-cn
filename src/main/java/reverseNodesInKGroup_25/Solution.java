...
public class Solution {
    public ListNode reverseKGroup(ListNode head, int k) {
        ...
        ListNode nextStart = curEnd.next;
        while (curEnd != null) {
            step--;
            if (step == 0) {
                ListNode curTail = curStart;
                nextStart = curEnd.next;
                ListNode curNext = curStart.next;
                curStart.next = null;
                while (curNext != nextStart) {
                    ListNode temp = curNext.next;
                    curNext.next = curStart;
                    curStart = curNext;
                    curNext = temp;
                }
                prevTail.next = curStart;
                curStart = nextStart;
                prevTail = curTail;
                curEnd = nextStart;
                step = k;
