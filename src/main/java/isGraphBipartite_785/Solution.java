...
public class Solution {
    ...
    public boolean isBipartite(int[][] graph) {
        ...
        for (int i = 0; i < graph.length; i++) {
            ...
            if (!isValidColor(graph, colors, i, WHITE)) {
                return false;
            }
        }
        return true;
    ...
    private boolean isValidColor(int[][] graph, int[] colors, int index, int color) {
        if (colors[index] != COLOR_NONE) {
            return color == colors[index];
        }
        colors[index] = color;
        for (int i = 0; i < graph[index].length; i++) {
            if (!isValidColor(graph, colors, graph[index][i], 1 - color)) {
                return false;
            }
        }
        return true;
